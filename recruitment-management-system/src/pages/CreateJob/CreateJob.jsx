...
import { useAddPositionMutation, useGetPositionListQuery } from "../../api/positionApi";
  ...
  const addNewPositionSubmit=()=>{
	...
	return (
		<div className="add-new-position-container">
			<FormInput label={"Enter new position"} value={position} handleChange={handleFieldChange}/>
			<Button text="Add" className={"add-new-position-button"} handleSubmit={addNewPositionSubmit}/>
		</div>
	);
};
...
const CreateJob = () => {
  ...
  useEffect(()=>{
    if(!isLoading ) {
      let positionFieldsDummy = data.map((obj) => ({ value: obj.name, display: obj.name }))
